...
from .lowpass import lowpass_filter, lowpass_filters, LowPassFilter,  LowPassFilters, LowPassFiltersNew  # noqa
...
class HighPassFiltersNew(torch.nn.Module):
    ...
    def __init__(self, cutoffs: torch.Tensor, stride: int = 1, pad: bool = True,
                 zeros: float = 8, fft: Optional[bool] = None):
        super().__init__()
        self._lowpasses = LowPassFiltersNew(cutoffs, stride, pad, zeros, fft)
...
def highpass_filters_new(input: torch.Tensor,  cutoffs: torch.Tensor,
                     stride: int = 1, pad: bool = True,
                     zeros: float = 8, fft: Optional[bool] = None):
    """
    Functional version of `HighPassFilters`, refer to this class for more information.
    """
    return HighPassFiltersNew(cutoffs, stride, pad, zeros, fft).to(input)(input)
...
def highpass_filter_new(input: torch.Tensor,  cutoff: torch.Tensor,
                    stride: int = 1, pad: bool = True,
                    zeros: float = 8, fft: Optional[bool] = None):
    """
    Functional version of `HighPassFilter`, refer to this class for more information.
    Output will not have a dimension inserted in the front.
    """
    return highpass_filters_new(input, cutoff, stride, pad, zeros, fft)[0]
