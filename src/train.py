...
from config import BATCH_SIZE, EPOCHS, LEARNING_RATE, DEBUG_MODE, REGRESSION_LOSS_FACTOR, \
    SPECTROGRAM_LOSS_FACTOR, PRINT_TRAIN_STATS, ARCHITECTURE, DATASET_MODE, DATASET_TYPE, SYNTH_TYPE, \
    ...
    NUM_EPOCHS_TO_SAVE_MODEL, FREQ_REINFORCE_LOSS_FACTOR, OVERRIDE_OPTIMIZER, SAMPLE_RATE, PRESET
    ...
    for target_signal, target_params_dic in data_loader:
        ...
        if SYNTH_TYPE == 'MODULAR':
            ...
            if SYNTH_TYPE == 'OSC_ONLY':
                ...
                if ARCHITECTURE == 'PARAMETERS_ONLY':
                    if FREQ_PARAM_LOSS_TYPE == 'CE':
                ...
                elif ARCHITECTURE == 'SPEC_NO_SYNTH':
            ...
            # if MODEL_FREQUENCY_OUTPUT == 'SINGLE', Spectrograms are
            # compared just as accuracy measure, without training
            if (ARCHITECTURE == 'FULL' or ARCHITECTURE == 'SPECTROGRAM_ONLY') \
                    or MODEL_FREQUENCY_OUTPUT == 'SINGLE' or MODEL_FREQUENCY_OUTPUT == 'WEIGHTED':

                denormalized_output_dic = normalizer.denormalize(output_dic)
                denormalized_output_dic['osc1_freq'] = torch.clamp(denormalized_output_dic['osc1_freq'],
                                                                   min=0, max=20000)
