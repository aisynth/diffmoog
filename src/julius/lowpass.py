...
class LowPassFiltersNew(torch.nn.Module):
    """
    Bank of low pass filters. Note that a high pass or band pass filter can easily
    be implemented by substracting a same signal processed with low pass filters with different
    frequencies (see `julius.bands.SplitBands` for instance).
    ...
    def __init__(self, cutoffs: torch.Tensor, stride: int = 1, pad: bool = True,
                 zeros: float = 8, fft: Optional[bool] = None):
        super().__init__()
        self.cutoff = cutoffs
        if torch.is_tensor(self.cutoff):
            self.cutoff_float = self.cutoff.item()
        else:
            self.cutoff_float = self.cutoff
        if self.cutoff_float < 0:
            raise ValueError("Minimum cutoff must be larger than zero.")
        if self.cutoff_float > 0.5:
            raise ValueError("A cutoff above 0.5 does not make sense.")
        self.stride = stride
        self.pad = pad
        self.zeros = zeros
        self.half_size = int(zeros / self.cutoff_float / 2)
        if fft is None:
            fft = self.half_size > 32
        self.fft = fft
        window = torch.hann_window(2 * self.half_size + 1, periodic=False)
        ...
        if self.cutoff_float == 0:
            ...
            filter_ = 2 * self.cutoff * window * sinc(2 * self.cutoff * math.pi * time)
            # Normalize filter to have sum = 1, otherwise we will have a small leakage
            # of the constant component in the input signal.
            filter_ /= filter_.sum()
        filters.append(filter_)
...
def lowpass_filters_new(input: torch.Tensor,  cutoffs: torch.Tensor,
    ...
    Functional version of `LowPassFilters`, refer to this class for more information.
    """
    return LowPassFiltersNew(cutoffs, stride, pad, zeros, fft).to(input)(input)
...
def lowpass_filter_new(input: torch.Tensor,  cutoff: torch.Tensor,
                   stride: int = 1, pad: bool = True,
                   zeros: float = 8, fft: Optional[bool] = None):
    """
    Same as `lowpass_filters` but with a single cutoff frequency.
    Output will not have a dimension inserted in the front.
    """
    return lowpass_filters_new(input, cutoff, stride, pad, zeros, fft)[0]
