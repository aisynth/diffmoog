...
class LitModularSynth(LightningModule):
    ...
    def __init__(self, train_cfg, device, tuning_mode=False):
        ...
        self.synth = SynthModular(preset=train_cfg.synth.preset, synth_constants=synth_constants,
                                  device=device)
        ...
        if train_cfg.loss.get('spec_loss_type', None) is not None:
        ...
        if train_cfg.loss.get('control_spec_preset', None) is not None:
    ...
    def in_domain_step(self, batch, log: bool = False, return_metrics=False):
        ...
        if self.current_epoch < self.cfg.loss.spectrogram_loss_warmup_epochs and not return_metrics:
            ...
            pred_final_signal, pred_signals_through_chain = self.generate_synth_sound(predicted_params_full_range,
                                                                                      batch_size)
            if self.cfg.loss.get("use_chain_loss", False):
            ...
            elif self.spec_loss is not None:
    ...
    def _balance_losses(self, parameters_loss, spectrogram_loss, log=False):
        ...
        parameters_loss_decay_factor = cfg.get("min_parameters_loss_decay", 0)
        ...
        weighted_spec_loss = cfg.get("spectrogram_loss_weight", 0) * spectrogram_loss * spec_loss_rampup_factor
    ...
    def _log_recursive(self, items_to_log: dict, tag: str, on_epoch=False):
        if isinstance(items_to_log, (float, np.float32, np.float64)) or \
                isinstance(items_to_log, (int, np.int32, np.int64)):
