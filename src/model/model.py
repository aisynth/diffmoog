...
class SynthNetwork(nn.Module):
    ...
    def __init__(self, cfg, synth_preset: str, device, backbone='resnet'):
        super().__init__()

        self.preset = synth_presets_dict.get(synth_preset, None)
        if cfg.synth.use_multi_spec_input:
            in_channels = cfg.synth.n_specs_input
        else:
            in_channels = 1
        ...
        if backbone in ['lstm', 'gru']:
            ...
            self.backbone.fc = nn.Linear(num_filters, LATENT_SPACE_SIZE)
