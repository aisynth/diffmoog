...
from model.model import SimpleSynthNetwork, ConditionalSynthNetwork
...
def train_single_epoch(model,
    ...
    with tqdm(data_loader, unit="batch") as tepoch:
        for target_signal, target_param_dict, signal_index in tepoch:
            ...
            # -----------Run Model-----------------
            conditional_vector = vectorize_unpredicted_params(target_param_dict, model.preset, device)
            output_params = model(transformed_signal, conditional_vector)
...
def run(args):
    ...
    if model_cfg.model_type == 'simple':
        n_conditional_parameters = count_unpredicted_params(synth_cfg.preset, model_cfg.preset)
        synth_net = ConditionalSynthNetwork(n_conditional_parameters, model_cfg.preset, synth_cfg, cfg, device,
                                            backbone=model_cfg.backbone).to(device)
    else:
        raise NotImplementedError("only ConditionalSynthNetwork supported at the moment")
